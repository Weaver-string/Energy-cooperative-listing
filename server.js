...
const RESEND_FROM = process.env.RESEND_FROM || "CoopGrid <onboarding@resend.dev>";
...
async function startServer() {
  await ensureStorage();
  server.listen(PORT, () => {
    console.log(`CoopGrid server running at ${PUBLIC_BASE_URL}`);
...
async function notifyAdmin(request) {
  ...
  const subject = `New CoopGrid listing request: ${request.orgName}`;
  const text = [
    "A cooperative has requested listing access on CoopGrid.",
    "",
...
async function notifyProfileSubmitted(request, profile) {
  const approveUrl = `${PUBLIC_BASE_URL}/api/listing-requests/${encodeURIComponent(request.id)}/approve?token=${encodeURIComponent(request.token)}`;
  const subject = `CoopGrid profile ready for review: ${profile.name}`;
  const text = [
    "A cooperative profile draft has been submitted on CoopGrid.",
    "",
...
async function ensureStorage() {
  ...
  const pool = getPool();
  await pool.query(`
    CREATE TABLE IF NOT EXISTS coopgrid_store (
...
async function readRecords(collection) {
  ...
  const result = await getPool().query(
    "SELECT data FROM coopgrid_store WHERE collection = $1 ORDER BY updated_at DESC",
    [collection],
  );
...
async function writeRecords(collection, values) {
  ...
  try {
    ...
    await client.query("DELETE FROM coopgrid_store WHERE collection = $1", [collection]);
    for (const value of values) {
      await client.query(
        "INSERT INTO coopgrid_store (collection, id, data, updated_at) VALUES ($1, $2, $3, NOW())",
